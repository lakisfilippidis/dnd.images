// Фильтр и группировка карточек — черт (featList) и снаряжения (weaponList,
// armorList), см. filterToolbar в eleventy.config.js.
//
// Оси фильтра берутся из чипов тулбара (data-axis): у черт это группа, сфера
// влияния и доступность, у оружия — группа, владение и вид урона. Внутри оси
// выбор работает как ИЛИ, между осями — И: «Клинок или Шпага» и при этом
// «Бой». Пустая ось не фильтрует. Страница класса приходит с уже нажатым
// чипом своего класса, поэтому фильтр применяется сразу, а не только по клику.
//
// Группировка — последний ряд, та же ось, но не сужает список, а раскладывает
...
//
// Каждый тулбар работает со своим списком — карточками контейнера
// .feat-cards--filtered в том же родителе, так что на одной странице их
...
document.querySelectorAll(".feat-filter").forEach((toolbar) => {
  ...
  const cards = [...(container?.querySelectorAll(".feat-card") ?? [])];
  if (cards.length === 0) return;
  const AXES = [...new Set([...toolbar.querySelectorAll("[data-axis]")].map((b) => b.dataset.axis))];
