...
- **Rogue feats:** the rogue class (`src/Classes/rogue/`) has no subclasses — players pick one feat per level. Feats live in `src/_data/rogueFeats.js` (groups, spheres of influence, requirements); shortcodes in `eleventy.config.js` render them as cards: `{% featCards "group" %}` / `{% featLegend %}` on the class page, `{% featPicks "id:level:note; ..." %}` on personality pages (relative asset/link paths assume depth-two pages)
